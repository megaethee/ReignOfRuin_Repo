...
public class Drunkard : MonoBehaviour
{
    ...
    private void Awake()
    {
        troop.troopStats.path = TroopStats.Path.Drunk;
    }
    ...
    public void Recalc()
    {
        ...
        troop.troopStats.path = TroopStats.Path.Across;

        troop.troopStats.xPosition = Mathf.RoundToInt(transform.parent.position.x);
        troop.troopStats.yPosition = Mathf.RoundToInt(transform.parent.position.z); 

        troop.finalTargCord = troop.troopStats.TargCordCompiler();
        StartCoroutine(troop.MoveOnGrid());
